<?php
class mutiara extends CI_Model{
	function __construct(){
		parent::__construct();
		$this->load->library('pagination');
	}
	function get_mutiara($base_url,$status=false){
		$this->db->start_cache();
			// Where
			$this->db->where('role_posting','mutiara');
			if($status){
		...
		if(post('add')){
			if(post('content_posting')){
				$data = array(
					...
					'role_posting' => 'mutiara'
				);
				$this->db->insert('posting',$data);
				$this->all->setMsg('success','Berhasil tambah kata mutiara');
				$_POST = '';
			}else{
				$this->all->setMsg('error','Kata mutiara harus diisi');
			}
		}
		// Edit
		if(post('edit')){
			if(post('content_posting')){
				...
				$this->all->setMsg('success','Berhasil edit kata mutiara');
			}else{
				$this->all->setMsg('error','Kata mutiara harus diisi');
			}
		}
