...
<div class='row'>
	<div class='title'>
		<h3><?=$act?> Kata Mutiara <a x href='<?=$link;?>' class='btn btn-inverse btn-xs'><i class='glyphicon glyphicon-arrow-left'></i> Back</a></h3>
	</div>
	<?php $this->all->getMsg();?>
	<form id='posting' method='post'>
		...
		<div class='form-group'>
			<label>Riwayat</label>
			<input type='text' name='title_posting' class='form-control' value='<?=$row['title_posting'];?>' placeholder='Riwayat' />
		</div>
		<div class='form-group'>
			<label>Kata Mutiara</label>
			<textarea id='wysihtml5' style='height:150px' name='content_posting' class='form-control' placeholder='Kata Mutiara'><?=$row['content_posting'];?></textarea>
		</div>
		<button type='submit' class='btn btn-primary'><?=$act?> Kata Mutiara</button>
	</form>
</div>
<?php }else{ ?>
<!--View Page-->
<div class='row'>
	...
	<div class='row' style='margin-bottom:10px;padding:0px 15px;'>
		<a x href='<?=$link;?>add' class='btn btn-primary' style='margin:0px 0px 10px'>+ Tambah kata mutiara</a>
	...
	?>
		<tr>
			...
			<td><?=word_limiter($row['content_posting'],4);?></td>
			<td>
				<div class='btn-group'>
				<?php
